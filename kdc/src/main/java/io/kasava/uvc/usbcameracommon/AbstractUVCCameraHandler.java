...
package io.kasava.uvc.usbcameracommon;
...
import io.kasava.uvc.encoder.MediaEncoder;
import io.kasava.uvc.encoder.MediaMuxerWrapper;
//import io.kasava.uvc.encoder.MediaSurfaceEncoder;
//import io.kasava.uvc.encoder.MediaVideoBufferEncoder;
import io.kasava.uvc.encoder.MediaVideoEncoder;
...
abstract class AbstractUVCCameraHandler extends Handler {
			...
			synchronized (thread.mSync) {
				...
				if (!isCameraThread()) {
					...
					try {
						...
						Log.e(TAG, "stopPreview()::failed to wait");
					}
				}
			}
		}
	...
	public void startRecording(File outputPath) {
		checkReleased();
		mUvcOutputFile = outputPath;
	...
	public void handleMessage(final Message msg) {
		...
		case MSG_CAPTURE_START:
			thread.handleStartRecording(mUvcOutputFile);
			break;
	...
	static final class CameraThread extends Thread {
		...
		public AbstractUVCCameraHandler getHandler() {
			...
			synchronized (mSync) {
				...
				try {
					...
					Log.e(TAG, "getHandler()::failed to wait");
				}
			}
			return mHandler;
		}
		...
		private void handleStartRecording(File path) {
			...
			try {
				...
				final MediaMuxerWrapper muxer = new MediaMuxerWrapper(path.getAbsolutePath());
				//MediaVideoBufferEncoder videoEncoder = null;
				switch (mEncoderType) {
				case 1:	// for video capturing using MediaVideoEncoder
					Log.v(TAG_THREAD, "handleStartRecording()::MediaVideoEncoder");
				...
				case 2:	// for video capturing using MediaVideoBufferEncoder
					Log.v(TAG_THREAD, "handleStartRecording::MediaVideoBufferEncoder");
				...
				default:
					Log.v(TAG_THREAD, "handleStartRecording::MediaSurfaceEncoder");
